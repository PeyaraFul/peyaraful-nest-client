    ...
    <nav className="sticky top-0 z-50 border-b border-default-200/50 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold tracking-tight">Peyaraful Nest</h1>
            <p className="text-xs text-default-500">Find your perfect home</p>
          </div>
        </Link>
        ...
        <div className="hidden items-center gap-3 md:flex">
          {!user ? (
            <>
              ...
              <Link href="/register" color="primary" radius="full">
                register
              </Link>
            </>
          ) : (
      ...
      <div
        className={`overflow-hidden transition-all duration-300 md:hidden ${
          isOpen ? "max-h-[500px] border-t border-default-200" : "max-h-0"
        }`}
      >
          ...
          <div className="mt-4 border-t border-default-200 pt-4">
            {!user ? (
              <div className="space-y-2">
                <Link href="/login" variant="light" className="w-full">
                  Login
                </Link>

                <Link href="/register" color="primary" className="w-full">
                  register
                </Link>
              </div>
            ) : (
